...
WORLD = {
    ...
    "rooms": {
        "village": {"desc": "A worn village square ringed by timber houses.",
                    "exits": ["market", "forest", "bridge", "tavern"], "npcs": ["elder"]},
        "market":  {"desc": "A cramped market stall hung with lanterns and trinkets.",
                    "exits": ["village"], "items": ["torch", "potion"], "npcs": ["merchant"]},
        "tavern":  {"desc": "A low, smoky taproom thick with the smell of ale.",
                    "exits": ["village"], "npcs": ["barkeep"]},
        "forest":  {"desc": "A dim forest, roots crawling underfoot.",
                    "exits": ["village", "cave", "grove"]},
        "cave":    {"desc": "A damp cave mouth, low growling echoing within.",
                    "exits": ["forest"], "items": ["key"], "enemy": "wolf"},
        "grove":   {"desc": "A trampled grove of broken saplings and churned earth.",
                    "exits": ["forest"], "enemy": "bear"},
        "bridge":  {"desc": "A mossy stone bridge over a black river.",
                    "exits": ["village", "ruins"]},
        "ruins":   {"desc": "Toppled columns around a sealed stone shrine.",
                    "exits": ["bridge", "crypt", "chapel"], "items": ["amulet"]},
        "crypt":   {"desc": "A pitch-dark crypt, air thick with old dust.",
                    "exits": ["ruins"], "enemy": "guardian"},
        "chapel":  {"desc": "A roofless chapel of cracked pews and a cold altar.",
                    "exits": ["ruins", "sanctum"], "items": ["ward"], "npcs": ["priest"]},
        "sanctum": {"desc": "A sealed inner sanctum where the air does not move.",
                    "exits": ["chapel"], "enemy": "wraith"},
    },
    "enemies": {
        "wolf":     {"hp": 14, "attack": 4},
        "bear":     {"hp": 20, "attack": 5},
    ...
    "npcs": {
        ...
        "barkeep":  {"role": "a broad tavern-keeper with an ear for every rumour",
                     "voice": "a tavern-keeper who deals in gossip and bounties on the beasts that trouble the roads"},
        "priest":   {"role": "a hooded priest tending the chapel's old rites",
...
OBJECTIVES = {
    "key":      {"kind": "fetch", "reveal_by": "elder",
                 "lead": "a key lies in the cave past the forest, with a wolf set to guard it"},
    "amulet":   {"kind": "fetch", "reveal_by": "elder",
                 "lead": "the amulet rests in the ruins, sealed behind a locked shrine"},
    "torch":    {"kind": "fetch", "reveal_by": "merchant",
                 "lead": "a torch waits in the market, a light for the dark places"},
    "ward":     {"kind": "fetch", "reveal_by": "priest",
                 "lead": "a warding charm sits in the chapel beside the ruins"},
    "guardian": {"kind": "kill",  "reveal_by": "elder",
                 "lead": "a guardian holds the crypt, and the crypt is pitch dark"},
    "bear":     {"kind": "kill",  "reveal_by": "barkeep",
                 "lead": "a great bear prowls the grove off the forest"},
    "wraith":   {"kind": "kill",  "reveal_by": "priest",
                 "lead": "a wraith haunts the sanctum, sealed beyond the chapel"},
}
...
def _validate_quests():
    ...
    for name, o in OBJECTIVES.items():
        ...
        assert (o["kind"] == "kill") == (G.nodes[name]["type"] == "enemy"), f"objective '{name}' kind mismatches its node"
