...
def test_unaffordable_cast_falls_back_to_attack():
    gs = players.new_game(
        [
            players.make_player(
                "X", "fighter", is_agent=True, stats={"name": "x", "max_hp": 20, "attack": 8, "max_mana": 1}
            )
        ]
    )
...
def test_battle_menu_respects_resources():
    gs = players.new_game(
        [players.make_player("X", "mage", is_agent=True, stats={"name": "x", "max_hp": 16, "attack": 6, "max_mana": 5})]
    )
    gs.inventory = []
...
def test_outlook_reads_the_race():
    gs = players.new_game(
        [
            players.make_player(
                "Frail", "mage", is_agent=True, stats={"name": "m", "max_hp": 14, "attack": 10, "max_mana": 2}
            )
        ]
    )
    gs.inventory = []
    ...
    tank = players.new_game(
        [players.make_player("Tank", "bruiser", is_agent=True, stats={"name": "t", "max_hp": 30, "attack": 10})]
    )
